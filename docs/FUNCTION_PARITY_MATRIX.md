...
- Firebase `lineWebhook`: verifies signature and logs events only.
...
| `isDuplicate` | Prevent duplicate LINE message processing | not started |
...
| `handleTextMessage` | Main text command and chat flow | not started |
...
| `replyToLine` / `pushMessage` | Reply and push messages | not started |
