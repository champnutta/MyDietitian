...
If any manual gate flag is used, also pass `--evidence-file docs/MANUAL_UAT_EVIDENCE.md`; the packet will validate the evidence file in pre-migration phase, requiring real LINE media, LIFF auth, rollback values, required `pass` rows, and owner sign-off.
