...
The final migration command only accepts a readiness packet JSON generated by `npm run migration:readiness-packet` within the last 6 hours for the same Firebase project, with completed evidence, all manual gates, all automated checks, no blockers, and a Google Sheet source fingerprint that still matches the live Sheet at write time.
