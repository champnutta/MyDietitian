...
The readiness packet JSON must be generated by `npm run migration:readiness-packet` for the same Firebase project within 6 hours of the migration window, have no blockers, include a completed evidence file, include every required manual gate as pass, include every automated pre-cutover check as pass, and show a clean Firestore target snapshot.
